...
func (s *StudentHandler) GetStatusStudents(active bool) ([]schemas.Student, error) {
	statusStudents := []schemas.Student{}
	err := s.DB.Where("active = ?", active).Find(&statusStudents)
	return statusStudents, err.Error
